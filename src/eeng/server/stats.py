...
class Assessment:
    ...
    def add_producers(self):
        ...
        self._producers = ee.Feature(
            None,
            {"producers": self._matrix.producersAccuracy().toList().flatten().slice(1)},
        )
        return self
    ...
    def add_consumers(self):
        ...
        self._producers = ee.Feature(
            None,
            {"consumers": self._matrix.consumersAccuracy().toList().flatten().slice(1)},
        )
        return self
    ...
    def build(self) -> ee.FeatureCollection:
        ...
        self._table = ee.FeatureCollection(
            [v for v in self.__dict__.values() if isinstance(v, ee.Feature)]
        )
        return self
